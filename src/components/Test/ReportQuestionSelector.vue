<template>
  <div>
    <div
      ...
      class="relative flex w-full items-center px-3 py-3.5 border rounded-2xl border-divider/50 bg-white cursor-pointer"
    >
      <span
        class="flex w-full h-6 overflow-hidden font-normal leading-6 font-compact text-17 text-txt"
        >{{
          !isSet
            ? "Выберите вопрос"
            : `[${types[props.questions[dropdownOption].type]}] ${
                props.questions[dropdownOption].txt
              }`
        }}</span
      >
      <ChevronDown />
