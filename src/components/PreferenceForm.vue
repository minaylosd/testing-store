<template>
  <form class="grid w-full grid-cols-2 px-6 pt-5 pb-6 bg-white gap-x-6 rounded-3xl">
    ...
    <div class="w-full">
      ...
      <input type="text" id="question" value="Что вы любите больше"
        class="block w-full px-3 py-3.5 border border-divider/50 font-compact font-normal text-17 leading-6 rounded-2xl text-txt bg-tertiary mb-[13px]"
        required />
      ...
      <label for="required-question"
        ...
        <div class="flex flex-col">
          <div v-for="(item, index) in filesArr" :key="index" :style="{ order: index }">
        <label :for="`object-${index}`" class="block mb-1 text-sm font-normal font-compact text-greytxt">{{ index + 1 }}
          объект</label>
        <input type="text" :id="`object-${index}`" @input="inputItemName(index, $event.target.value)"
          placeholder="Название"
          :value="item.name? item.name : ''"
          class="block w-full px-3 py-3.5 border border-divider/50 font-compact font-normal text-17 leading-6 rounded-2xl text-greytxt bg-tertiary mb-[13px]"
          required />
      </div>
        </div>
      

      <input type="checkbox" id="scramble-answers" checked class="option custom-checkbox" />
      <label for="scramble-answers"
